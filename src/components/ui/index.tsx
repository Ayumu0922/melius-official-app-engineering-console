...
export function SearchShell({ children, ...props }: HTMLAttributes<HTMLButtonElement> & ChildrenProps) {
  return (
    <button
      ...
      className="flex w-full items-center gap-3 rounded-xl bg-slate-100 px-3.5 py-2.5 text-left text-sm text-slate-500 transition-colors hover:bg-slate-200/70 dark:bg-white/6 dark:text-slate-400 dark:hover:bg-white/10"
    >
      {children}
    </button>
  );
...
export function NavButton({ dataId, roleName, selected, badge, badgeTone, onClick, icon, children }: NavButtonProps) {
  ...
  return (
    <button
      ...
      data-melius-ui-role={roleName}
      onClick={onClick}
      className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-slate-700 transition-colors hover:bg-slate-100 hover:text-slate-950 dark:text-slate-300 dark:hover:bg-white/8 dark:hover:text-white"
    >
      <span className="shrink-0">{icon}</span>
      <span className="min-w-0 flex-1 truncate text-left">{children}</span>
...
export function IconButton({ dataId, roleName, label, children, onClick, disabled, type = 'button' }: ButtonProps) {
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-950 disabled:cursor-not-allowed disabled:opacity-50 dark:text-slate-400 dark:hover:bg-white/8 dark:hover:text-white"
    >
      {children}
    </button>
  );
...
export function SegmentButton({ dataId, roleName, selected, onClick, children }: SegmentButtonProps) {
  ...
  return (
    <button
      ...
      data-melius-ui-role={roleName}
      onClick={onClick}
      className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-500 transition-colors hover:bg-slate-200 hover:text-slate-900 dark:bg-white/6 dark:text-slate-400 dark:hover:bg-white/10 dark:hover:text-white"
    >
      {children}
    </button>
  );
